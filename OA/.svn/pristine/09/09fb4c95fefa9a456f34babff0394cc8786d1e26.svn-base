...
//				args.put("name", name);
				String processinstanceid = myworkflowservice.SaveAndStart("WF002", id, user.get("userid").toString(),vars,args.get("name").toString(),"TB_OA_CONTRACT");
...
//				args.put("name", name);
				String processinstanceid = myworkflowservice.SaveAndStart("WF005", id, user.get("userid").toString(),vars,args.get("name").toString(),"TB_OA_CONTRACT");
				args.put("#SQL", "update TB_OA_CONTRACT set fprocessinstanceid=#{processinstanceid} where fid=#{id}");
				args.put("processinstanceid", processinstanceid);
			}else if(args.get("type1").equals("3")){
//				String name = "腾天事业部合同 "+number;
//				args.put("name", name);
				String processinstanceid = myworkflowservice.SaveAndStart("WF011", id, user.get("userid").toString(),vars,args.get("name").toString(),"TB_OA_CONTRACT");
				args.put("#SQL", "update TB_OA_CONTRACT set fprocessinstanceid=#{processinstanceid} where fid=#{id}");
				args.put("processinstanceid", processinstanceid);
			}
