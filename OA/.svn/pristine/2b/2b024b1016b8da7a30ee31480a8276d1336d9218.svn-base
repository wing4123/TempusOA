...
<form id="form_bill" class="form-horizontal" role="form">
    		...
    		<input type="hidden" id="id" name="id" value="${returnObject.reimbursement.FID}"/>
    		<div class="form-group">
	            <label class="col-md-2 control-label">标题<span class="required">*</span></label>
	            <div class="col-md-8">
	                <input class="form-control" name="name" value="${returnObject.reimbursement.FNAME}" required />
	            </div>
	        </div>   			
   			<div class="form-group">
	            ...
	            <div class="col-md-10">
	                <div class="mt-radio-inline" style="padding-bottom: 0px;" id="type">
	                    <label class="mt-radio">
	                        <input type="radio" name="type" value="1" onchange="relation(this.value)" <c:if test="${returnObject.reimbursement.FTYPE==1}">checked</c:if> />礼品及业务招待费用/差旅费用审批/交通补助<span></span>
	                    </label>
	                    <label class="mt-radio">
	                        <input type="radio" name="type" value="2" onchange="relation(this.value)" <c:if test="${returnObject.reimbursement.FTYPE==2}">checked</c:if> />零星开支（办公用品、水电费、快递费等未经事前审批的报销事项）<span></span>
	                    </label>
	                </div>
	            </div>
	        </div>
   			...
   			<%-- <div class="form-group">
           		...
           		<div class="col-md-3">
					<input id="payee" value='${returnObject.reimbursement.USER_NAME}' class="form-control" readonly="readonly" style="cursor: pointer;" onclick="showusermodal(this)" required />
					<input name="payee" value='${returnObject.reimbursement.USER_ID}' type="hidden">
				</div>
   			</div> --%>
   			<div class="form-group">
                        ...
                        <c:forEach items="${returnObject.entry}" var="row1" varStatus="S">
                        	<div class="row">
								...
								<div class="col-md-6">
									...
									<input name="content" class="form-control" value="${row1.FCONTENT}" />
								</div>
								<div class="col-md-2">
									<label class="control-label">金额<span class="required">*</span></label>
									<input name="amount" class="form-control currency" value="${row1.FAMOUNT}"  />
								</div>
								<div class="col-md-1">
								    <label class="control-label">&nbsp;</label>
	...
	$('#form_bill').validate({
        ...
        submitHandler: function (form) { //表单校验通过并提交表单
        	if($("#list").children().length==0){
        		...
        		$(form).find("select[name='relationcurrency']").removeAttr("disabled");
...
var changecompany = function(companyid){
	if(companyid!=""){
		$.get("../Reimbursement/getDepartmentList",{"companyid": companyid},function(data){
...
var initalrelationbill = function(){
	...
	datatable.DataTable({
		...
		//dom: '<<t>lp>',
		ajax: {
	        url: '../Reimbursement/getRelationBill'
		},
	    columns: [
