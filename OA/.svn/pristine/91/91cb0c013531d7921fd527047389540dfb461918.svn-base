...
*  
*/
@Service("FindTellerTaskListener")
public class FindTellerTaskListener implements Serializable, TaskListener {  
    ...
    public void notify(DelegateTask task) {
    	...
    	while (divisionid!=null){
        	...
        	if(division!=null){
        		List<HashMap<String, Object>> userlist = basicservice.SelectListBySql("select a.user_id from tb_user a left join TB_OA_userposition b on b.fuserid=a.user_id left join TB_OA_position c on fid=b.fpositionid where a.attribute13='"+division.get("FID")+"' and c.fname='出纳'");
