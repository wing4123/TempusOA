		...
		<form id="form3" class="form-horizontal" role="form">
			...
			<div class="form-group">
				<div class="form-group-inline">
					...
					<div class="col-md-4">
						<select class="form-control" name="paymentaccount" required>
				...
				<div class="form-group-inline">
					...
					<div class="col-md-4">
						<div class="input-group date date-picker" data-date-format="yyyy-mm-dd" >
					       <input name="appaymentdate" class="form-control" required />
					       <span class="input-group-btn" style="vertical-align: top;">
					            <button class="btn default" type="button">
					                <i class="fa fa-calendar"></i>
					            </button>
...
<script>
	$(function(){
		$('#form3 .date-picker').datepicker({
	        autoclose: true,
	        language: "<%=locale %>",
	        todayBtn: "linked",
	        todayHighlight: true
	    });
		$('form3 .date-picker').datepicker("setDate",new Date());
		...
		$('#form3').validate({
	        ...
	        highlight: function (element) { // hightlight error inputs
	            $(element).closest('.form-group-inline').addClass('has-error'); // set error class to the control group
	        },
	        unhighlight: function (element) { // revert the change done by hightlight
	            $(element).closest('.form-group-inline').removeClass('has-error'); // set error class to the control group
	        },
	        submitHandler: function (form) { //表单校验通过并提交表单
	        	App.blockUI({animate: true});
        		var data = $(form).serializeJSON();
        		$.post("../Payment/Teller",data,function(rs){
        			if(rs.code=="S"){
        				approval(btn);
        			}else{
